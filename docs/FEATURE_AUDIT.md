...
- Finds R even off the PATH: the chosen R (`VOWELCHEMY_RSCRIPT` or Set up tools), the chosen tool env / app venv / PATH, then the platform's install locations (Windows registry + Program Files, macOS `R.framework` + Homebrew + `/opt/R`, Linux `/usr/lib/R` + `/opt/R`) and every conda environment; each R is probed once (version, first library, phontrast/phonJSD version) and the first with a usable phontrast wins (≥ 2.3.1 required; an older phontrast or legacy **phonJSD** install is reported with an update hint naming the R that was checked). `install_plan()` installs phontrast from CRAN into that R's user library. Generates and runs an R driver over an exported CSV subset that calls `phontrast()` per vowel pair (bandwidth, density, `min_tokens`, bootstrap options pass through) and binds `pillai_overlap(…, proportion_standardized = TRUE)` fields plus `pillai_null_p95` onto the wide table; graceful notes when R or output is missing.
